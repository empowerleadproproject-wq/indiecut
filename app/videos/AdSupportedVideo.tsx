...
export default function AdSupportedVideo({src,poster,breakMode='interval',intervalMinutes=6,customBreaks=''}:{src:string;poster?:string;breakMode?:BreakMode;intervalMinutes?:number;customBreaks?:string}){
 const contentRef=useRef<HTMLVideoElement>(null);const adRef=useRef<HTMLVideoElement>(null);const pendingBreakRef=useRef<PendingBreak|null>(null);const resumeAtRef=useRef(0);const wasPlayingRef=useRef(false);
 ...
 async function restoreContent(){
  ...
  setActiveAd(null);pendingBreakRef.current=null;setCountdown(0);
  if(wasPlayingRef.current){try{await video.play()}catch{}}
 }
 function launchAd(key:string,ad:Ad){
  const video=contentRef.current,adVideo=adRef.current;if(!video||!adVideo||activeAd)return;
  pendingBreakRef.current=null;resumeAtRef.current=video.currentTime;wasPlayingRef.current=!video.paused&&!video.ended;setPlayedBreaks(x=>x.includes(key)?x:[...x,key]);
  video.pause();setActiveAd(ad);setAdIndex(x=>x+1);setCountdown(0);
  try{adVideo.currentTime=0}catch{}
  requestAnimationFrame(()=>{adVideo.play().catch(()=>{void restoreContent()})});
 }
 ...
 function failAd(){const pending=pendingBreakRef.current;if(pending){pendingBreakRef.current=null;setPlayedBreaks(x=>x.includes(pending.key)?x:[...x,pending.key]);return}if(activeAd)void restoreContent()}
 ...
 return <div style={{position:'relative',width:'100%',background:'#000',overflow:'hidden'}}>
  <video ref={contentRef} src={src} poster={poster} controls={!activeAd} playsInline preload="metadata" onTimeUpdate={onTime} onPlay={()=>{if(!activeAd)wasPlayingRef.current=true}} onPause={()=>{if(!activeAd)wasPlayingRef.current=false}} style={{width:'100%',aspectRatio:'16/9',maxHeight:720,background:'#000',objectFit:'contain',display:'block'}}/>
  {adForElement&&<div aria-hidden={!activeAd} style={{position:'absolute',inset:0,zIndex:10,background:'#000',display:'grid',placeItems:'center',opacity:activeAd?1:0,pointerEvents:activeAd?'auto':'none',transition:'opacity 90ms linear'}}>
   <video ref={adRef} key={adForElement._id} src={adForElement.creative_url} preload="auto" playsInline onCanPlay={onAdCanPlay} onCanPlayThrough={onAdCanPlay} onTimeUpdate={adTime} onEnded={()=>void restoreContent()} onError={failAd} controls={false} style={{width:'100%',height:'100%',objectFit:'contain'}}/>
