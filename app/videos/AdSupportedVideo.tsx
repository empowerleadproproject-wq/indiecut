...
export default function AdSupportedVideo({src,poster,breakMode='interval',intervalMinutes=6,customBreaks=''}:{src:string;poster?:string;breakMode?:BreakMode;intervalMinutes?:number;customBreaks?:string}){
 ...
 return <div ref={playerRef} style={{position:'relative',width:'100%',height:fullscreen?'100vh':'auto',background:'#000',overflow:'hidden',display:'grid',placeItems:'center'}}>
  <video ref={contentRef} src={src} poster={poster} controls={!activeAd} playsInline preload="metadata" onTimeUpdate={checkBreak} onSeeked={checkBreak} style={{width:'100%',height:fullscreen?'100%':'auto',aspectRatio:fullscreen?undefined:'16/9',maxHeight:fullscreen?'100vh':720,background:'#000',objectFit:'contain',display:'block'}}/>
  {!activeAd&&<button type="button" onClick={toggleFullscreen} aria-label={fullscreen?'Exit fullscreen':'Fullscreen'} style={{position:'absolute',right:12,bottom:fullscreen?18:48,zIndex:30,width:38,height:34,border:0,borderRadius:4,background:'rgba(0,0,0,.62)',color:'#fff',fontSize:20,cursor:'pointer',display:'grid',placeItems:'center'}}>{fullscreen?'↙':'⛶'}</button>}
  {activeAd&&<div style={{position:'absolute',inset:0,zIndex:40,background:'#000',display:'grid',placeItems:'center'}}>
  ...
  {breakMode!=='none'&&ads.length>0&&!activeAd&&<div style={{position:'absolute',right:fullscreen?58:52,top:10,padding:'5px 8px',background:'rgba(0,0,0,.6)',color:'#fff',fontSize:10,fontWeight:800,zIndex:25}}>{scheduleLabel}</div>}
 </div>
}
