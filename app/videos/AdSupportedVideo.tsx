...
export default function AdSupportedVideo({src,poster,breakMode='interval',intervalMinutes=6,customBreaks=''}:{src:string;poster?:string;breakMode?:BreakMode;intervalMinutes?:number;customBreaks?:string}){
 const contentRef=useRef<HTMLVideoElement>(null);const adRef=useRef<HTMLVideoElement>(null);const resumeAtRef=useRef(0);const playedRef=useRef<Set<string>>(new Set());const activeRef=useRef(false);
 const [ads,setAds]=useState<Ad[]>([]);const [activeAd,setActiveAd]=useState<Ad|null>(null);const [adIndex,setAdIndex]=useState(0);const [countdown,setCountdown]=useState(0);
 ...
 async function resumeProgram(){
  const program=contentRef.current;activeRef.current=false;setActiveAd(null);setCountdown(0);if(!program)return;
  try{program.currentTime=resumeAtRef.current}catch{}
  try{await program.play()}catch{}
 }
 async function playAd(key:string){
  if(activeRef.current||playedRef.current.has(key)||!ads.length)return;
  const program=contentRef.current;if(!program)return;
  const ad=ads[adIndex%ads.length];if(!ad)return;
  playedRef.current.add(key);activeRef.current=true;resumeAtRef.current=program.currentTime;program.pause();setActiveAd(ad);setAdIndex(i=>i+1);setCountdown(0);
 }
 useEffect(()=>{
  if(!activeAd)return;const adVideo=adRef.current;if(!adVideo)return;
  const start=()=>{try{adVideo.currentTime=0}catch{};adVideo.play().catch(()=>void resumeProgram())};
  if(adVideo.readyState>=2)start();else adVideo.addEventListener('canplay',start,{once:true});
  return()=>adVideo.removeEventListener('canplay',start);
 },[activeAd]);
 function checkBreak(){
  const program=contentRef.current;if(!program||activeRef.current||!ads.length||breakMode==='none')return;
  const t=program.currentTime;
  if(breakMode==='interval'){
   const breakNo=Math.floor(t/interval);if(breakNo>=1){const key=`i:${breakNo}`;if(!playedRef.current.has(key))void playAd(key)}return;
  }
  for(const at of customTimes){const key=`c:${at}`;if(t>=at&&!playedRef.current.has(key)){void playAd(key);break}}
 }
 ...
 return <div style={{position:'relative',width:'100%',background:'#000',overflow:'hidden'}}>
  <video ref={contentRef} src={src} poster={poster} controls={!activeAd} playsInline preload="metadata" onTimeUpdate={checkBreak} onSeeked={checkBreak} style={{width:'100%',aspectRatio:'16/9',maxHeight:720,background:'#000',objectFit:'contain',display:'block'}}/>
  {activeAd&&<div style={{position:'absolute',inset:0,zIndex:20,background:'#000',display:'grid',placeItems:'center'}}>
