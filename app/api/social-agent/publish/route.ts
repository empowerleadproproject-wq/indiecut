...
async function postFacebook(article:any,caption:string,link:string){
 const pageId=process.env.META_PAGE_ID,token=process.env.META_PAGE_ACCESS_TOKEN;if(!pageId||!token)return {ok:false,reason:'Facebook not connected'};
 ...
 const r=await fetch(`https://graph.facebook.com/${pageId}/feed`,{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded'},body});const j=await r.json().catch(()=>({}));return r.ok?{ok:true,id:j.id}:{ok:false,reason:j?.error?.message||'Facebook post failed'};
}

async function postInstagram(article:any,caption:string,link:string){
 const ig=process.env.INSTAGRAM_USER_ID,token=process.env.META_PAGE_ACCESS_TOKEN;if(!ig||!token)return {ok:false,reason:'Instagram not connected'};
 const image=String(article.featured_media_url||'');if(!/^https?:\/\//i.test(image)||/\.(mp4|webm|mov|m4v)(\?|$)/i.test(image))return {ok:false,reason:'Instagram auto-post requires a public image on the article'};
 const createBody=new URLSearchParams({access_token:token,image_url:image,caption:`${caption}${link?`\n\n${link}`:''}`});
 const c=await fetch(`https://graph.facebook.com/${ig}/media`,{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded'},body:createBody});const cj=await c.json().catch(()=>({}));if(!c.ok||!cj.id)return {ok:false,reason:cj?.error?.message||'Instagram media creation failed'};
 const pBody=new URLSearchParams({access_token:token,creation_id:cj.id});const p=await fetch(`https://graph.facebook.com/${ig}/media_publish`,{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded'},body:pBody});const pj=await p.json().catch(()=>({}));return p.ok?{ok:true,id:pj.id}:{ok:false,reason:pj?.error?.message||'Instagram publish failed'};
}
...
export async function POST(request:Request){
 ...
 const db=createServiceClient(url,key,{auth:{persistSession:false,autoRefreshToken:false}});const input=await request.json().catch(()=>({}));const articleId=String(input.article_id||'');if(!articleId)return NextResponse.json({error:'Missing article_id'},{status:400});
 const [{data:article},{data:setting}]=await Promise.all([db.from('articles').select('*').eq('id',articleId).eq('status','published').maybeSingle(),db.from('site_settings').select('setting_value').eq('setting_key','admin_social_agent').maybeSingle()]);
 if(!article)return NextResponse.json({error:'Published article not found'},{status:404});let settings:any={enabled:false,auto_post_on_publish:false,facebook:true,instagram:true,tiktok:false,include_link:true,include_hashtags:true};try{if(setting?.setting_value)settings={...settings,...JSON.parse(setting.setting_value)}}catch{}
 ...
 const link=settings.include_link===false?'':`${SITE_URL}/articles/${encodeURIComponent(article.slug)}`;const caption=await makeCaption(article,settings);const results:any={};
 if(settings.facebook)results.facebook=await postFacebook(article,caption,link);
 if(settings.instagram)results.instagram=await postInstagram(article,caption,link);
