...
export async function POST(request:Request){
 ...
 for(const s of stories){
  ...
  const headline=String(s?.headline||'').trim();if(!headline)continue;
  const slug=cleanSlug(headline);const duplicate=(existingRows||[]).some((x:any)=>cleanSlug(x.slug||x.headline)===slug||String(x.headline||'').trim().toLowerCase()===headline.toLowerCase());if(duplicate){rejected.push({headline,reason:'Duplicate'});continue}
  ...
  created.push({headline,slug,category:payload.category,sources:sources.length,image:imageUrl||null,image_source:imageSource||null});
 }
