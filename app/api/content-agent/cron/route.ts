...
export async function GET(request:Request){
 ...
 await recordStatus(supabase,{status:'running',date:now.date,frequency,scheduled_hour:targetHour,topic,requested:count,pipeline:'two-pass research + standards review + story-level duplicate/material-update gate'});

 const {data:existingRows}=await supabase.from('articles').select('headline,slug,subheadline,subject_name,body,created_at,published_at').order('created_at',{ascending:false}).limit(400);
...
Cover movies, television, music, actors, musicians, filmmakers, creators, celebrity news, independent entertainment and culture, with strong editorial attention to Black entertainment and African-American culture when relevant. INDIE SPOTLIGHT: actively discover rising independent music artists, filmmakers and creators with a real current momentum/news peg. Verify that they are genuinely independent before assigning a spotlight category. Use indie-music-spotlight for verified independent music artists, indie-film-spotlight for verified independent filmmakers, and indie-creator-spotlight for other verified independent creators. Do not label someone independent when ownership, label, studio or representation evidence is unclear; use normal categories instead. Use live web search. NEVER use blind items, anonymous gossip, rumor aggregation, social-media speculation, fabricated quotes, unverified relationship claims or unsupported controversy. Prefer primary sources such as official statements, studios, networks, labels, festivals, direct interviews, court/public records, awards bodies and direct announcements, then reputable entertainment trades and major newsrooms. Require at least two credible source URLs whenever possible, preferably from genuinely different sources. Cross-check names, dates, titles, release information, awards, numbers, legal claims, deaths, health information and quotations. If sources disagree, mark the uncertainty instead of smoothing it over. If a major claim cannot be corroborated, mark it not_verified.

COVERAGE FRESHNESS RULE — IMPORTANT:
- The same person or artist may be covered again when something genuinely different happens.
- Do NOT publish the same underlying event, situation or angle again just because another outlet has a new article or the headline can be rewritten.
- A continuing situation may be covered again ONLY when a material factual development happened after Indie Cut's prior coverage and advances the story.
- A meaningfully different event or angle about the same person is allowed.

Avoid duplicating or lightly reframing existing Indie Cut coverage: ${existing||'none yet'}.
...
Return ONLY valid JSON: {"stories":[{"headline":"","subheadline":"","category":"movies|tv|music|culture|independent|celebrity","subject_name":"","body":"4-7 substantial paragraphs","sources":["https://...","https://..."],"featured_image_url":"https://...","featured_image_source_url":"https://...","verification_status":"verified|not_verified","verification_note":"specific internal explanation","why_now":"one sentence news peg","news_score":0}]}.`;
 ...
 for(const s of stories){
  ...
  created.push({headline,slug,category:payload.category,sources:sources.length,source_hosts:diverseSources,image:imageUrl||null,why_now:String(s?.why_now||''),news_score:Number(s?.news_score||0),coverage_decision:coverage.decision,coverage_reason:coverage.reason,matched_headline:coverage.matched_headline});
 }
 ...
 await recordStatus(supabase,{status:created.length?'success':'completed_no_articles',date:now.date,frequency,scheduled_hour:targetHour,requested:count,created_count:created.length,rejected_count:rejected.length,created,rejected,pipeline:'two-pass research + standards review + story-level duplicate/material-update gate'});
 return NextResponse.json({ok:true,date:now.date,frequency,requested:count,created,rejected,editorial_pipeline:'two-pass research + standards review + story-level duplicate/material-update gate'});
}