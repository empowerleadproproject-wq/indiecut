...
export async function GET(request:Request){
 ...
 await recordStatus(supabase,{status:'running',date:now.date,frequency,scheduled_hour:targetHour,topic,requested:count});
 ...
 let ai:Response;
 try{
  ai=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{authorization:`Bearer ${apiKey}`,'content-type':'application/json'},body:JSON.stringify({model:'gpt-5.6-luna',tools:[{type:'web_search'}],input:prompt,max_output_tokens:28000})});
 }catch(e:any){await recordStatus(supabase,{status:'failed',date:now.date,reason:`OpenAI request error: ${e?.message||'unknown error'}`});return NextResponse.json({error:'Research request could not be sent.'},{status:502})}
 const aiJson=await ai.json().catch(()=>({}));
 if(!ai.ok){const reason=aiJson?.error?.message||'Research request failed.';await recordStatus(supabase,{status:'failed',date:now.date,reason});return NextResponse.json({error:reason},{status:502})}

 let parsed:any;
 try{parsed=parseJson(textFromResponse(aiJson))}catch{await recordStatus(supabase,{status:'failed',date:now.date,reason:'The research model returned invalid JSON.'});return NextResponse.json({error:'The research model returned invalid JSON.'},{status:502})}
