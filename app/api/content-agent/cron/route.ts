...
const DEFAULTS={enabled:true,default_topic:'top current entertainment news involving Black culture, movies, television, music, celebrities and independent creators',default_count:3,require_multiple_sources:true,reject_rumors:true,cron_hour:9,cron_timezone:'America/New_York'};
...
function localParts(timeZone:string){const parts=new Intl.DateTimeFormat('en-US',{timeZone,year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',hourCycle:'h23'}).formatToParts(new Date());const get=(t:string)=>parts.find(p=>p.type===t)?.value||'';return {date:`${get('year')}-${get('month')}-${get('day')}`,hour:Number(get('hour'))}}
...
export async function GET(request:Request){
 ...
 if(last?.setting_value===now.date)return NextResponse.json({ok:true,skipped:'Already ran today.'});
 ...
 await supabase.from('site_settings').upsert({setting_key:LAST_RUN_KEY,setting_value:now.date,updated_at:new Date().toISOString()},{onConflict:'setting_key'});
 return NextResponse.json({ok:true,date:now.date,requested:count,created,rejected});
}
