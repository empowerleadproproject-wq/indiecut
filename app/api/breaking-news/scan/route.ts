...
export async function POST(request:Request){
 ...
 const prompt=`You are Indie Cut's BREAKING NEWS RADAR. Search the live web for current entertainment developments. Prioritize stories first reported or materially updated in the last ${hours} hours. If there are not enough strong leads in that window, include major entertainment developments from the last 24 hours that are still actively developing or newly relevant. Coverage focus: ${focus}. Return up to ${count} useful leads ranked by urgency and editorial value. Do not invent or predict news. Exclude rumors, blind items, unsupported social posts, fan speculation, recycled stories with no new development, and opinion pieces presented as breaking news. Prefer primary sources and major entertainment/news outlets. Mark VERIFIED when the central claim is supported by at least two credible source URLs, or one authoritative primary source plus a credible corroborating report. Otherwise mark needs_verification, but still include the lead if it is newsworthy and has at least one credible source. Avoid Indie Cut stories already covered: ${existing||'none'}. Return ONLY valid JSON with this shape: {"leads":[{"headline":"suggested original Indie Cut headline","summary":"1-3 sentence factual summary","category":"movies|tv|music|culture|independent|celebrity","subject_name":"","why_breaking":"why this matters now","source_published_at":"ISO-8601 timestamp if available","urgency_score":0,"verification_status":"verified|needs_verification","verification_note":"brief explanation","sources":["https://...","https://..."],"image_url":"optional trustworthy editorial image URL or blank"}]}. Current UTC time: ${now}.`;
