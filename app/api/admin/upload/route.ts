...
export async function POST(request:Request){
 ...
 if(file.size>max)return NextResponse.json({error:isVideo?'Video uploads must be 50 MB or smaller.':'This upload is too large.'},{status:413});
 ...
 const {data:pub}=db.storage.from(bucket).getPublicUrl(path);return NextResponse.json({publicUrl:pub.publicUrl,path,bucket,mediaType:isVideo?'video':isAudio?'audio':'image'});
}
