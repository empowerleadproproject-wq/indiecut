...
export async function POST(request:Request){
 ...
 if(isVideo||isAudio){
  ...
  try{await s3.send(new PutObjectCommand({Bucket:cfg.bucket,Key:probeKey,Body:'ok',ContentType:'text/plain'}));await s3.send(new DeleteObjectCommand({Bucket:cfg.bucket,Key:probeKey})).catch(()=>{})}catch(e:any){return NextResponse.json({error:`Cloudflare R2 write check failed. ${r2Error(e)} Check the R2 Account ID, Access Key ID, Secret Access Key, and bucket name in Vercel.`},{status:502})}
  const signedUrl=await getSignedUrl(s3,new PutObjectCommand({Bucket:cfg.bucket,Key:path}),{expiresIn:3600});
  return NextResponse.json({provider:'r2',path,signedUrl,uploadUrl:'/api/admin/r2-upload',publicUrl:`${cfg.publicBase}/${path}`,mediaType:isVideo?'video':'audio',method:'PUT',contentType:mime});
 }
