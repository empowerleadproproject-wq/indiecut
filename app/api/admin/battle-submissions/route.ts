...
async function contests(db:any){const {data}=await db.from('battle_contests').select('id,title,slug,status,qualifying_starts_at,qualifying_ends_at').neq('status','completed').order('created_at',{ascending:false});return data||[]}
...
export async function POST(request:Request){
 ...
 if(action==='approve'){
  ...
  const {data:entry,error}=await db.from('battle_entries').insert({contest_id:contestId,artist_name:item.artist_name,slug,genre:item.genre||null,city:item.city||null,bio:item.bio||null,image_url:item.image_url||null,track_title:item.track_title||null,track_url:item.track_url||null,track_cover_url:item.image_url||null,active:true}).select('id,slug').single();if(error)return NextResponse.json({error:error.message},{status:400});
  rows[index]={...item,status:'approved',approved_to_contest_id:contestId,battle_entry_id:entry.id,updated_at:new Date().toISOString()};const {error:saveError}=await writeSubmissions(db,rows);if(saveError)return NextResponse.json({error:saveError.message},{status:400});return NextResponse.json({ok:true,submissions:await publicRows(db),contests:await contests(db),entry});
 }
