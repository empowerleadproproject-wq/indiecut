...
export async function POST(request:Request){
 ...
 if(action==='approve'){
  ...
  const {data:contest}=await db.from('battle_contests').select('id,title,slug').eq('id',contestId).maybeSingle();if(!contest)return NextResponse.json({error:'Competition not found.'},{status:404});
