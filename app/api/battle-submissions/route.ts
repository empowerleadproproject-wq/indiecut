...
export async function POST(request:Request){
 ...
 const artist_name=String(body.artist_name||'').trim().slice(0,120),email=String(body.email||'').trim().slice(0,180),track_title=String(body.track_title||'').trim().slice(0,160),genre=String(body.genre||'').trim().slice(0,80),city=String(body.city||'').trim().slice(0,120),bio=String(body.bio||'').trim().slice(0,1200),social_handle=String(body.social_handle||'').trim().slice(0,180),track_url=String(body.track_url||'').trim(),image_url=String(body.image_url||'').trim();
 if(!artist_name||!email||!track_title||!track_url)return NextResponse.json({error:'Artist name, email, song title and song upload are required.'},{status:400});if(!/^\S+@\S+\.\S+$/.test(email))return NextResponse.json({error:'Enter a valid email address.'},{status:400});if(body.rights!==true)return NextResponse.json({error:'Please confirm you have the rights to submit this recording.'},{status:400});
 ...
 const row={id:crypto.randomUUID(),artist_name,email,track_title,genre,city,bio,social_handle,track_url,image_url,status:'pending',ip_hash,created_at:new Date().toISOString(),updated_at:new Date().toISOString()};const {error}=await writeJsonSetting(db,'battle_submissions',[row,...submissions].slice(0,500));if(error)return NextResponse.json({error:error.message},{status:400});return NextResponse.json({ok:true,id:row.id,message:'Submission received for Phase One.'});
}
