...
export default function IndieCutRadioPlayer(){
 const [cfg,setCfg]=useState(defaults),[open,setOpen]=useState(false),[playing,setPlaying]=useState(false),[error,setError]=useState('');const audio=useRef<HTMLAudioElement>(null);
 useEffect(()=>{fetch('/api/radio',{cache:'no-store'}).then(r=>r.json()).then(j=>setCfg({...defaults,...j})).catch(()=>{})},[]);
 useEffect(()=>{const a=audio.current;if(!a)return;const onPlay=()=>{setPlaying(true);setError('')},onPause=()=>setPlaying(false),onError=()=>{setPlaying(false);setError('The radio stream is temporarily unavailable.')};a.addEventListener('play',onPlay);a.addEventListener('pause',onPause);a.addEventListener('error',onError);return()=>{a.removeEventListener('play',onPlay);a.removeEventListener('pause',onPause);a.removeEventListener('error',onError)}},[cfg.stream_url]);
 if(!cfg.enabled)return null;
 const toggle=async()=>{const a=audio.current;if(!a||!cfg.stream_url)return;try{if(a.paused){setError('');await a.play();setOpen(true)}else a.pause()}catch{setError('Your browser could not start the radio stream.')}};
 return <><audio ref={audio} src={cfg.stream_url||undefined} preload="none"/><div style={{position:'fixed',right:18,bottom:18,zIndex:9998,fontFamily:'Arial,sans-serif'}}>
  {open&&<div style={{width:330,maxWidth:'calc(100vw - 36px)',background:'#0b0b0b',color:'#fff',border:'1px solid #2a2a2a',boxShadow:'0 18px 55px rgba(0,0,0,.4)',padding:18,marginBottom:10}}>
   ...
   {!cfg.stream_url&&<div style={{fontSize:12,color:'#aaa',marginTop:14}}>Station stream is being configured.</div>}{error&&<div role="status" style={{fontSize:12,color:'#ffb3b3',marginTop:14}}>{error}</div>}
  </div>}
