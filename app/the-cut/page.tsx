...
export default function TheCutPage(){
  ...
  return <main className={styles.hall}>
    ...
    <div id="cut-hall-tabs" className={styles.hallTabs}><b>HALLWAY</b><span>UPCOMING</span><span>MY ROOMS</span></div>
    <section id="cut-room-feed" className={styles.roomFeed}>{hallLoading?<div className={styles.roomLoading}>Loading live rooms…</div>:hallRoom?<article className={styles.roomCard}><div><small>🔴 LIVE　•　{hallRoom.club}</small><h2>{hallRoom.title}</h2><div className={styles.preview}><div className={styles.faceStack}>{hallRoom.people.slice(0,4).map((p,i)=><span key={p.id} style={{zIndex:4-i}}>{p.promo?.image?<img src={p.promo.image} alt={p.promo.name}/>:p.avatarIsUrl?<img src={p.avatar} alt={p.name}/>:p.avatar}</span>)}</div><div><strong>{lead?.promo?.name||lead?.name||'Live now'}</strong><p>{hallRoom.people.length?hallRoom.people.slice(1,4).map(p=>p.promo?.name||p.name).join('　'):'Open room — listen in live'}</p><p>{hallRoom.listening} listening　•　{speakers} speakers</p></div></div></div><button className={styles.joinCard} onClick={()=>enterRoom(hallRoom.slug)}>▥　Enter Room</button></article>:<div className={styles.roomLoading}>No live room is available right now.</div>}</section>
  </main>;
}
...
function LiveRoom({slug,back}:{slug:string;back:()=>void}){
  ...
  return <main className={styles.space}>
    ...
    {promoViewer&&<PromoDetail promo={promoViewer.promo} own={promoViewer.own} remaining={promoViewer.own?promoRemaining:undefined} close={()=>setPromoViewer(null)}/>} 
...
function PromoDetail({promo,own,remaining,close}:{promo:Promo;own:boolean;remaining?:number;close:()=>void}){
