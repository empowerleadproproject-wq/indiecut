...
export default function RoomHostGate({children}:{children:ReactNode}){
  ...
  async function checkout(choice:'entry'|'product'){
    ...
    try{const r=await fetch('/api/the-cut/checkout',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({roomSlug:slug,choice,name:attendeeName,email:attendeeEmail,phone:attendeePhone,emailMarketingOptIn:emailOptIn,smsMarketingOptIn:smsOptIn})});const j=await r.json();if(!r.ok)throw new Error(j.error||'Checkout could not start');if(j.url)location.href=j.url;else throw new Error('Checkout URL unavailable')}catch(e:any){setError(e.message||'Checkout could not start');setPaying(false)}
  }
  ...
  return <main style={{minHeight:'100dvh',background:'#000',color:'#fff',display:'grid',placeItems:'center',padding:24,fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif',boxSizing:'border-box'}}>
    <section style={{width:'min(560px,100%)',textAlign:'center',background:'#0d0f13',border:'1px solid #262a31',borderRadius:24,padding:'34px 26px',boxShadow:'0 24px 80px rgba(0,0,0,.45)'}}>
      ...
      {state==='paywall'&&<div style={{margin:'16px 0 22px'}}><p style={{color:'#9da4af',lineHeight:1.5}}>Enter your information before checkout. Name and email are required. Phone is optional.</p><div style={{display:'grid',gap:9,textAlign:'left',marginBottom:14}}><input value={attendeeName} onChange={e=>setAttendeeName(e.target.value)} placeholder="Name *" autoComplete="name" style={{border:'1px solid #343a44',borderRadius:12,background:'#090c11',color:'#fff',padding:12,fontSize:15}}/><input value={attendeeEmail} onChange={e=>setAttendeeEmail(e.target.value)} placeholder="Email *" type="email" autoComplete="email" style={{border:'1px solid #343a44',borderRadius:12,background:'#090c11',color:'#fff',padding:12,fontSize:15}}/><input value={attendeePhone} onChange={e=>setAttendeePhone(e.target.value)} placeholder="Phone (optional)" type="tel" autoComplete="tel" style={{border:'1px solid #343a44',borderRadius:12,background:'#090c11',color:'#fff',padding:12,fontSize:15}}/><label style={{display:'flex',gap:8,alignItems:'flex-start',fontSize:12,color:'#9da4af'}}><input type="checkbox" checked={emailOptIn} onChange={e=>setEmailOptIn(e.target.checked)}/>Send me email updates from this room host.</label>{attendeePhone&&<label style={{display:'flex',gap:8,alignItems:'flex-start',fontSize:12,color:'#9da4af'}}><input type="checkbox" checked={smsOptIn} onChange={e=>setSmsOptIn(e.target.checked)}/>Send me text updates from this room host. Message/data rates may apply.</label>}</div><p style={{color:'#9da4af'}}>Choose how you want to enter.</p><div style={{display:'grid',gap:10}}>{premium?.admissionMode!=='purchase_required'&&<button disabled={paying||attendeeName.trim().length<2||!attendeeEmail.includes('@')} onClick={()=>checkout('entry')} style={{border:0,borderRadius:999,background:'#785cff',color:'#fff',padding:13,fontWeight:900,cursor:'pointer',opacity:(paying||attendeeName.trim().length<2||!attendeeEmail.includes('@'))?.55:1}}>Pay ${(Number(premium?.entryPriceCents||0)/100).toFixed(2)} Entry</button>}{premium?.admissionMode!=='entry_fee'&&<button disabled={paying||attendeeName.trim().length<2||!attendeeEmail.includes('@')} onClick={()=>checkout('product')} style={{border:'1px solid #444',borderRadius:999,background:'#171a20',color:'#fff',padding:13,fontWeight:900,cursor:'pointer',opacity:(paying||attendeeName.trim().length<2||!attendeeEmail.includes('@'))?.55:1}}>Buy {premium?.productName||'Package'} — ${(Number(premium?.productPriceCents||0)/100).toFixed(2)}</button>}</div>{premium?.productKind==='physical'&&<small style={{display:'block',color:'#777',marginTop:10}}>Shipping address is collected securely at checkout. The host fulfills physical orders.</small>}</div>}
