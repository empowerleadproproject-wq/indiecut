...
export default function SocialAgentSettings(){
 ...
 async function refresh(){const [s,c]=await Promise.all([fetch('/api/admin/data?section=social-agent'),fetch('/api/social-agent/status')]);const sj=await s.json();const cj=await c.json();if(s.ok&&sj.rows?.[0])setSettings({...defaults,...sj.rows[0]});if(c.ok)setStatus(cj)}
 ...
 return <section className="ic-module-panel"><h2>Social Media Agent</h2><p>Automatically turn published Indie Cut stories into platform-ready social posts. The article stays the source of truth; the agent only promotes published content.</p>
  ...
  <div className="ic-agent-results"><h3>Platform connections</h3>
   ...
   <p><strong>TikTok:</strong> {status?.tiktok?'Connected':'Needs TikTok Content Posting authorization/app approval'}</p>
