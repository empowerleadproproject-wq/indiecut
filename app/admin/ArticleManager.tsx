...
export default function ArticleManager(){
 ...
 async function spotlight(row:Article,category:string){setBusy(true);setMessage('Moving article into Indie Spotlight…');try{const data={...row,category};const r=await fetch('/api/admin/data',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({section:'articles',id:row.id,data})});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.error||'Unable to update spotlight tag');setMessage('Article added to Indie Spotlight.');await load()}catch(e:any){setMessage(e.message)}finally{setBusy(false)}}
 ...
 return <section className="ic-module-panel">
  ...
  <div className="ic-two"><label>Category<select value={form.category||'culture'} onChange={e=>setForm({...form,category:e.target.value})}><option>movies</option><option>tv</option><option>music</option><option>culture</option><option>independent</option><option>celebrity</option><option value="indie-music-spotlight">indie music spotlight</option><option value="indie-film-spotlight">indie filmmaker spotlight</option><option value="indie-creator-spotlight">indie creator spotlight</option></select></label><label>Subject / Person<input value={form.subject_name||''} onChange={e=>setForm({...form,subject_name:e.target.value})}/></label></div>
  ...
  <div style={{display:'grid',gap:12}}>{rows.map(row=><article key={row.id} style={{border:'1px solid #ddd',padding:14,display:'grid',gridTemplateColumns:'100px minmax(0,1fr) auto',gap:14,alignItems:'start'}}>{row.featured_media_url?<img src={row.featured_media_url} alt="" style={{width:100,height:78,objectFit:'cover'}}/>:<div style={{width:100,height:78,background:'#eee'}}/>}<div><div style={{fontSize:11,fontWeight:800,letterSpacing:'.08em'}}>{String(row.category||'').toUpperCase()} · {String(row.status||'draft').toUpperCase()}</div><h3 style={{margin:'5px 0'}}>{row.headline}</h3><div style={{fontSize:12,color:'#666'}}>{row.lead_video_url?'🎬 VIDEO ATTACHED':'No article video'} · {row.verification_status||'pending'}</div></div><div style={{display:'flex',gap:6,flexWrap:'wrap',justifyContent:'flex-end'}}><button onClick={()=>edit(row)}>EDIT</button><select aria-label="Move to Indie Spotlight" defaultValue="" disabled={busy} onChange={e=>{if(e.target.value)spotlight(row,e.target.value);e.currentTarget.value=''}}><option value="">ADD TO SPOTLIGHT…</option><option value="indie-music-spotlight">MUSIC ARTIST</option><option value="indie-film-spotlight">FILMMAKER</option><option value="indie-creator-spotlight">CREATOR</option></select>{row.status!=='published'&&<button onClick={()=>publish(row)} disabled={busy}>PUBLISH</button>}{row.status==='published'&&<button onClick={()=>retrySocial(row)} disabled={busy}>RETRY SOCIAL</button>}<button onClick={()=>remove(row.id)}>DELETE</button></div></article>)}</div>
 </section>
}
