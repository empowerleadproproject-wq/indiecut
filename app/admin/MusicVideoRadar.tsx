...
type VideoLead={artist_name:string;song_title:string;video_title?:string;genre?:string;artist_tier?:'independent'|'major'|'unknown';official_video_url:string;channel_name?:string;release_date?:string;view_count?:number;why_feature?:string;momentum_score?:number;thumbnail_url?:string;sources?:string[];discovered_at?:string};
...
export default function MusicVideoRadar(){
 const [focus,setFocus]=useState('Trending official music videos across R&B, hip-hop, soul, pop, Afrobeats and adjacent genres, from independent and major artists');
 ...
 async function loadLatest(){try{const r=await fetch('/api/music-video-radar/scan');const j=await r.json().catch(()=>({}));if(r.ok&&j.scan){setVideos((j.scan.videos||[]).filter((v:VideoLead)=>Number(v.view_count||0)>=14000));setLastScan(j.scan.scanned_at||'');if(j.scan.focus)setFocus(j.scan.focus);if(j.scan.days&&j.scan.days<=7)setDays(j.scan.days)}}catch{}}
 async function scan(){setBusy(true);setMessage('Searching for fresh trending official music videos with at least 14,000 verified views…');try{const r=await fetch('/api/music-video-radar/scan',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({focus,days,count})});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.error||'Music-video scan failed');setVideos(j.videos||[]);setLastScan(j.scanned_at||'');setMessage(`${(j.videos||[]).length} trending music-video lead(s) found with 14,000+ views.`)}catch(e:any){setMessage(e.message)}finally{setBusy(false)}}
 ...
 return <section className="ic-module-panel">
  <div className="ic-admin-eyebrow">TRENDING VIDEO DISCOVERY</div>
  <h2>Music Video Radar</h2>
  <p>Find fresh, official music videos from independent and major artists. Results must be no more than 7 days old and have at least <strong>14,000 verified views</strong>. Indie Cut embeds the official artist, label or Vevo upload instead of downloading copyrighted video files.</p>
  <div className="ic-two"><label>Discovery focus<input value={focus} onChange={e=>setFocus(e.target.value)}/></label><label>Release window<select value={days} onChange={e=>setDays(Number(e.target.value))}><option value={1}>Last 24 hours</option><option value={3}>Last 3 days</option><option value={7}>Last 7 days</option></select></label></div>
  ...
  <div className="ic-actions"><button onClick={scan} disabled={busy}>{busy?'SCANNING TRENDING VIDEOS…':'FIND TRENDING MUSIC VIDEOS'}</button><a className="ic-link-button" href="/videos">OPEN WATCH →</a></div>
  {lastScan&&<p><small>Latest scan: {fmt(lastScan)} · {videos.length} qualified leads · {independentCount} independent · minimum 14,000 views</small></p>}
  ...
  {videos.length>0&&<div className="ic-agent-results" style={{display:'grid',gap:16,marginTop:18}}>{videos.map((v,i)=>{const embed=youtubeEmbed(v.official_video_url);return <article className="ic-agent-card" key={`${v.official_video_url}-${i}`} style={{alignItems:'flex-start'}}>
   <div style={{flex:1,minWidth:0}}><div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}><span className="ic-status-pill">{String(v.artist_tier||'unknown').toUpperCase()}</span><strong>Momentum {Math.max(0,Math.min(100,Number(v.momentum_score||0)))}/100</strong><strong>{Number(v.view_count||0).toLocaleString()} views</strong></div><h3>{v.artist_name} — {v.song_title}</h3><p><strong>{v.genre||'Music video'}</strong>{v.channel_name?` · Official source: ${v.channel_name}`:''}{v.release_date?` · Released ${v.release_date}`:''}</p>{v.why_feature&&<p>{v.why_feature}</p>}
