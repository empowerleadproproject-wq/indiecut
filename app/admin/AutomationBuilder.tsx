...
const TRIGGERS=[
 ['manual','Manual Enrollment','Start when you manually add a contact'],
 ['contact_created','Contact Added','Start when a new CRM contact is created'],
 ['status_changed','Contact Status Changed','Start when a contact moves to a selected status'],
 ['tag_added','Tag Added','Start when a selected tag is added'],
...
export default function AutomationBuilder(){
 ...
 async function deleteWorkflow(){if(!workflow.id||!confirm('Delete this workflow?'))return;const j=await run('delete','delete_workflow',{workflowId:workflow.id});if(j)newWorkflow()}
 async function enroll(){if(!workflow.id||!enrollContact)return;const j=await run('enroll','enroll_contact',{workflowId:workflow.id,contactId:enrollContact});if(j)setEnrollContact('')}
