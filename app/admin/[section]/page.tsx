...
const sections:Record<string,{title:string;intro:string}>={
 ...
 artists:{title:'Artists',intro:'Review artist profile submissions, approve or reject new artists, and edit approved artist profiles.'},
...
export default async function AdminSectionPage({params}:{params:{section:string}}){
 ...
 const content=params.section==='artists'?<><BattleSubmissionsManager/><BattleArtistEditor/></>:params.section==='live-battles'?<><BattleRoomSponsors/><BattleLandingSettings/><BattleSubmissionsManager/><BattleArtistEditor/><BattleManager/></>:params.section==='crm'?<CrmManager/>:params.section==='music'?<MusicManager/>:params.section==='workflows'?<AutomationBuilderV5/>:params.section==='articles'?<ArticleManager/>:params.section==='breaking-news'?<BreakingNewsRadar/>:params.section==='independent-music'?<IndependentMusicRadar/>:params.section==='music-video-radar'?<MusicVideoRadar/>:params.section==='social-agent'?<SocialAgentSettings/>:params.section==='analytics'?<><WebsiteAnalytics/><SocialAnalytics/></>:params.section==='tracking-pixels'?<TrackingPixelsSettings/>:params.section==='advertising'?<AdvertisingManager/>:<SectionManager section={params.section}/>;
