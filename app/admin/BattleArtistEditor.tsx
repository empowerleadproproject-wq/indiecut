        ...
        return <article key={artist.id} style={{border:'1px solid #ddd',background:'#fff',padding:16}}>
          ...
          {open&&<div style={{marginTop:18,paddingTop:18,borderTop:'1px solid #ddd',display:'grid',gap:14}}>
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:12}}>
              ...
              <label>YouTube<input value={d.youtube_url||''} onChange={e=>patch(artist.id,'youtube_url',e.target.value)} placeholder="@channel or YouTube URL"/></label>
              <label>Facebook<input value={d.facebook_url||''} onChange={e=>patch(artist.id,'facebook_url',e.target.value)} placeholder="page name or Facebook URL"/></label>
            </div>
