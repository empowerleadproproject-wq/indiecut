...
export default function SubmissionForm({open=true}:{open?:boolean}){
 ...
 async function submit(e:FormEvent<HTMLFormElement>){
  ...
  try{
   ...
   const track_url=await directUpload(track,'audio');const image_url=photo instanceof File&&photo.size?await directUpload(photo,'image'):'';setMessage('Finishing your Phase One entry…');
   const payload={artist_name:String(form.get('artist_name')||''),email:String(form.get('email')||''),track_title:String(form.get('track_title')||''),genre:String(form.get('genre')||''),city:String(form.get('city')||''),social_handle:String(form.get('social_handle')||''),bio:String(form.get('bio')||''),website:String(form.get('website')||''),rights:String(form.get('rights')||'')==='yes',track_url,image_url};
