...
function parseSponsors(value?:string|null){try{const rows=JSON.parse(value||'[]');return Array.isArray(rows)?rows:[]}catch{return[]}}

export default async function BattleRoomPage(){
 const db=battleDb();
 const [{data:rows},{data:sponsorSetting}]=await Promise.all([
  db.from('battle_contests').select('*').in('status',['live','scheduled','completed']).order('created_at',{ascending:false}).limit(25),
  db.from('site_settings').select('setting_value').eq('setting_key','admin_battle_room_sponsors').maybeSingle()
 ]);
 const sponsors=parseSponsors(sponsorSetting?.setting_value).filter((x:any)=>x?.image_url&&x?.active!==false);
