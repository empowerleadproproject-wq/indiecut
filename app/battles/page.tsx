...
async function landingData(){
 const db=battleDb();
 const [{data:setting},{data:articles},{data:contests}]=await Promise.all([
  ...
  db.from('battle_contests').select('slug').in('status',['qualifying','scheduled','live']).order('updated_at',{ascending:false}).limit(1)
 ]);
...
export default async function BattlesLandingPage(){
 const {content,currentBattleSlug}=await landingData();
