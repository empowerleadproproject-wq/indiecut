...
type Props={slug:string;initialData:any};
const videoExt=/\.(mp4|webm|mov|m4v)(\?|$)/i;

export default function BattleLiveClient({slug,initialData}:Props){
 ...
 return <>
  ...
  {contest.current_segment==='voting'&&round&&<section className={styles.section}><div className={styles.sectionHead}><div><div className={styles.eyebrow}>ROUND {round.round_number}</div><h2>Vote now</h2></div><div className={styles.countdown}>{seconds}s left</div></div><div className={styles.votePanel}><div className={styles.voteChoice}><h3>{entryA?.artist_name}</h3><p>{round.track_a_title||entryA?.track_title||'Artist A track'}</p><button className={styles.voteButton} disabled={voting||seconds===0} onClick={()=>castVote(entryA.id)}>VOTE {entryA?.artist_name?.toUpperCase()}</button></div><div className={styles.voteChoice}><h3>{entryB?.artist_name}</h3><p>{round.track_b_title||entryB?.track_title||'Artist B track'}</p><button className={styles.voteButton} disabled={voting||seconds===0} onClick={()=>castVote(entryB.id)}>VOTE {entryB?.artist_name?.toUpperCase()}</button></div></div>{message&&<div className={styles.notice}>{message}</div>}</section>}
