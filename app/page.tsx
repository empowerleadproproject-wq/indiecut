...
export default async function Home(){
 ...
 const accent=String(home.accent_color||'#d71920');const now=new Date().toISOString().slice(0,10);const liveAds=ads.filter(a=>isLiveAd(a,now));const rightRailAds=shuffled(liveAds.filter(a=>['right-rail','homepage'].includes(a.placement))).slice(0,4);const trendingPool=stories.slice(0,Math.min(6,stories.length));const lead=trendingPool.length?trendingPool[Math.floor(Math.random()*trendingPool.length)]:null;const secondary=stories.filter(s=>s.id!==lead?.id);const latest=secondary.slice(0,7);const below=secondary.slice(0,12);
