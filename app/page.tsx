...
function adIsVideo(a:any){return Boolean(a?._isVideo)||String(a?.creative_media_type||'').startsWith('video/')||video(a?.creative_url)}
function storagePathFromUrl(url:string){const marker='/storage/v1/object/public/indiecut-media/';const i=url.indexOf(marker);return i>=0?decodeURIComponent(url.slice(i+marker.length).split('?')[0]):''}
async function resolveAdMedia(client:any,a:any){const url=String(a?.creative_url||'');const path=String(a?.creative_storage_path||storagePathFromUrl(url));if(!path)return a;try{const {data,error}=await client.storage.from('indiecut-media').createSignedUrl(path,3600);if(!error&&data?.signedUrl)return {...a,creative_url:data.signedUrl}}catch{}return a}
...
export default async function Home(){
 ...
 const accent=String(home.accent_color||'#d71920');const now=new Date().toISOString().slice(0,10);const liveAds=ads.filter(a=>isLiveAd(a,now));const resolvedAds=await Promise.all(liveAds.map(a=>resolveAdMedia(client,a)));const typedAds=await Promise.all(resolvedAds.map(classifyAd));const rightRailAds=pickRailAds(typedAds.filter(a=>['right-rail','homepage'].includes(a.placement)),4);const trendingPool=stories.slice(0,Math.min(6,stories.length));const lead=trendingPool.length?trendingPool[Math.floor(Math.random()*trendingPool.length)]:null;const secondary=stories.filter(s=>s.id!==lead?.id);const latest=secondary.slice(0,7);const below=secondary.slice(0,12);
