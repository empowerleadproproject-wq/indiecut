...
async function classifyAd(a:any){
 if(adIsVideo(a))return {...a,_isVideo:true};
 const url=String(a?.creative_url||'');if(!url)return {...a,_isVideo:false};
 try{const r=await fetch(url,{method:'HEAD',cache:'no-store'});const type=String(r.headers.get('content-type')||'').toLowerCase();return {...a,_isVideo:type.startsWith('video/')};}catch{return {...a,_isVideo:false}}
}
...
export default async function Home(){
 ...
 const accent=String(home.accent_color||'#d71920');const now=new Date().toISOString().slice(0,10);const liveAds=ads.filter(a=>isLiveAd(a,now));const typedAds=await Promise.all(liveAds.map(classifyAd));const rightRailAds=pickRailAds(typedAds.filter(a=>['right-rail','homepage'].includes(a.placement)),4);const trendingPool=stories.slice(0,Math.min(6,stories.length));const lead=trendingPool.length?trendingPool[Math.floor(Math.random()*trendingPool.length)]:null;const secondary=stories.filter(s=>s.id!==lead?.id);const latest=secondary.slice(0,7);const below=secondary.slice(0,12);
