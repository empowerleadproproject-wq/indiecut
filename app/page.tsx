...
function pickRailAds(items:any[],limit=4){const local=shuffled(items.filter(isLocalTarget));const global=shuffled(items.filter(a=>!isLocalTarget(a)));const pool=[...local,...global];const vid=pool.find(adIsVideo);const rest=pool.filter(a=>a!==vid);return (vid?[vid,...rest]:rest).slice(0,limit)}
...
export default async function Home(){
 ...
 return <main className="site-shell ic-homepage" style={{'--ic-accent':accent} as any}><PublicHeader/>
  ...
  {spotlights.length>0&&<section className="ic-home-sections ic-spotlight-home"><div className="ic-section-title ic-spotlight-title"><span>INDIE SPOTLIGHT</span><a href="/indie-spotlight">VIEW ALL →</a></div><div className="ic-story-grid">{spotlights.map((s:any)=><article key={s.id}><a href={`/articles/${s.slug}`}>{s.featured_media_url&&<Media url={s.featured_media_url} alt={s.headline}/>}<span>{String(s.category||'INDIE SPOTLIGHT').replace(/-/g,' ').toUpperCase()}</span><h3>{s.headline}</h3>{s.subheadline&&<p>{s.subheadline}</p>}</a></article>)}</div></section>}
